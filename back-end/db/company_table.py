...
async def add_new_image(company_id:str, file_name:str, file_type:str, file_hash:str, full_path:str, status: str, match_field:str):
    response = supabase.rpc("exec_sql", {"sql": ADD_IMAGE_INTO_TABLE.format(company_id=company_id, file_name=file_name, file_type=file_type, file_hash=file_hash, full_path=full_path, status=status, match_field=match_field)}).execute()
    return response.data
