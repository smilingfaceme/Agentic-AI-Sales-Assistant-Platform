...
from db.company_table import get_all_messages, add_new_message, get_customer_by_id
...
def condition_workflow_function(blocks: list[dict], from_phone_number: str, source_phone_number: str, messages: list[dict], query: str, message_type: str, platform:str, company_schema: str, conversation_id: str):
    for block in blocks:
        if block["key"] == "message_filter.text":
            ...
            if block["settings"]["operator"] == "is":
                ...
                if block["settings"]["value"] == from_phone_number:
                    return None
        elif block["key"] == "check_customer_info":
            customers = get_customer_by_id(company_schema, conversation_id)
            if customers and customers[0]["customer_id"] and customers[0]["customer_name"] and customers[0]["customer_email"] and customers[0]["customer_phone"]:
                pass
            else:
                return None
    return True
...
async def action_workflow_function(
    ...
    for block in blocks:
        if block["key"] == "ai_reply":
            if message_type == "Image" and image_search_result and block["settings"]["value"] == "Find products with images":
                ...
                ai_response, extra_info = await ai_response_with_search(company_id, company_schema, conversation_id, query, memory, [block["settings"]["value"]])
            final_response.append([ai_response, extra_info])
