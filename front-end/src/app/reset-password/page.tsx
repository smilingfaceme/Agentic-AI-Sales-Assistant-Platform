...
export default function AcceptInvitePage() {
  ...
  const { isLoading: isResetingPassword, execute: executeResetPassword } = useApiCall();
    ...
    async function fetchResetpassword() {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE}/auth/forgot-password/validate?token=${token}`
        );
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    if (valid) {
      const result = await executeResetPassword(async () => {
      ...
      if (result.token) {
        showNotification("Successfully reset password!", 'success', true)
        ...
        showNotification("Failed to reset password. Please try again.", 'error', true)
      }
    }
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-blue-50 via-purple-50 to-pink-50">
        <div className="m-2 w-full max-w-md bg-white rounded-xl shadow-lg p-8 flex flex-col items-center">
          ...
          <p className="text-gray-600 mb-6 text-center">
            It looks like your invitation can’t be used anymore. Please resend the reset-password request.
          </p>
          <Link href="/" className="mb-4">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-blue-50 via-purple-50 to-pink-50">
      <div className="m-2 w-full max-w-md bg-white rounded-xl shadow-lg p-8 flex flex-col items-center">
        ...
        <h1 className="text-2xl md:text-3xl font-extrabold text-gray-700 mb-2">
          Reset Password
        </h1>
